...
ハザクラは、Gemini AIの全知性（Unified Context）を搭載した次世代作家支援プラットフォームです。プロジェクトごとにAIとの会話履歴、プロット、設定、執筆本文が完全に統合され、説明不要で文脈を理解する体験を提供します。
